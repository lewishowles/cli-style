...
import { diagnosticReport, profiles, resultTypes } from "../../src/index.js";
...
describe("diagnosticReport", () => {
		...
		const output = diagnosticReport(
			{
				...
				nextActions: ["Review failing tests", "Re-run unit diagnostics"],
				skippedChecks: [
					{
						name: "e2e",
	...
	test("Keeps CI output plain and grep-friendly", () => {
		...
		expect(output).toBe(
			["Project diagnostics", "", "Checks", "OK Success type-check - No errors"].join("\n"),
		);
	});
